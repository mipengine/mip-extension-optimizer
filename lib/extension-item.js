    ...
    return [
...

        {
            ...
            process: function (builder) {
                ...
                if (mainFile) {
                    var content = '\n\nrequire(["mip", "' + name + '"], function (mip, component) {\n'
                        + 'mip.registerMipElement("' + name + '", component';
                    if (mainStyleFile) {
                        var styleFile = builder.getFile(name + '/' + mainStyleFile);
                        content += ', ' + util.stringLiteralize(styleFile.getData());
                    }
                    content += ');\n});\n\n';


                    mainFile.setData(mainFile.getData() + content);
                }
